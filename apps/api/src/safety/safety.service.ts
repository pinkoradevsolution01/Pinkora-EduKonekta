...
export class SafetyService {
  ...
  private async safeguardingAccess(a: AuthContext) {
    ...
    if (!a.roles.includes(RoleCode.GUIDANCE))
      throw new ForbiddenException('Confidential safeguarding access is required');
    const access = await this.prisma.safeguardingAccess.findFirst({
      where: { schoolId, userId: a.userId, isActive: true },
    });
  ...
  private rateLimit(a: AuthContext) {
    ...
    if (entries.length >= 5)
      throw new HttpException(
        'Please wait before submitting another safety report',
        HttpStatus.TOO_MANY_REQUESTS,
      );
    entries.push(now);
    this.submissions.set(a.userId, entries);
  }
  async submit(a: AuthContext, input: CreateSafetyReportInput) {
    const schoolId = this.school(a);
    this.rateLimit(a);
    const reporter = await this.prisma.user.findFirst({
      where: { id: a.userId, status: 'ACTIVE', emailVerifiedAt: { not: null } },
      select: { email: true, displayName: true },
    });
    if (!reporter)
      throw new ForbiddenException('A verified school account is required to submit a report');
    const fingerprint = safetyFingerprint(schoolId, input.description, input.incidentDate);
    const duplicateCandidateCount = await this.prisma.safetyReport.count({
      where: {
        schoolId,
    ...
    const report = await this.prisma.safetyReport.create({
      data: {
        ...
        abuseIndicator: duplicateCandidateCount >= 3,
      },
    });
    await this.prisma.safetyReportUpdate.create({
      data: {
        schoolId,
        reportId: report.id,
        status: SafetyReportStatus.SUBMITTED,
        reporterNote: 'Your report was submitted confidentially.',
      },
    });
    await this.audit('SAFETY_REPORT_SUBMITTED', a, report.id, { category: report.category });
    await this.publisher.publish(
      createEvent('safety.report.submitted', report.id, schoolId, {
    ...
    return {
      ...
      status: report.status,
      confirmation:
        'Your confidential report has been submitted. If someone may be in immediate danger, contact local emergency services or a trusted adult now.',
    };
  }
  async mine(a: AuthContext) {
    const items = await this.prisma.safetyReport.findMany({
      where: { schoolId: this.school(a), reporterUserId: a.userId },
  ...
  async intake(a: AuthContext, id?: string) {
    ...
    const output = reports.map((report) => ({
      ...
      description: decryptProtected<string>(report.descriptionEncrypted),
      reporter: decryptProtected<{ displayName: string; email: string }>(
        report.protectedIdentityEncrypted,
      ),
      evidence: report.evidenceEncrypted ? decryptProtected(report.evidenceEncrypted) : null,
      status: report.status,
      duplicateCandidateCount: report.duplicateCandidateCount,
      abuseIndicator: report.abuseIndicator,
      updates: report.updates,
    }));
    for (const report of reports)
      await this.audit('SAFETY_REPORT_CONFIDENTIAL_ACCESSED', a, report.id);
    return id ? output[0] : output;
  }
  async update(a: AuthContext, id: string, input: UpdateSafetyReportInput) {
    await this.safeguardingAccess(a);
    const schoolId = this.school(a);
    const report = await this.prisma.safetyReport.findFirst({ where: { id, schoolId } });
    if (!report) throw new NotFoundException('Safety report not found');
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.safetyReportUpdate.create({
        data: {
          schoolId,
          reportId: id,
          status: input.status,
    ...
    await this.publisher.publish(
      createEvent('safety.report.status.updated', id, schoolId, { status: input.status }),
    );
  ...
  async signedEvidence(a: AuthContext, id: string) {
    ...
    if (!report?.evidenceEncrypted) throw new NotFoundException('Evidence not found');
    await this.audit('SAFETY_REPORT_EVIDENCE_LINK_ISSUED', a, id);
    return {
      url: `/api/v1/safety/reports/${id}/evidence?token=${signAttachment('safety-evidence', id, this.school(a), new Date(Date.now() + 60_000))}`,
      expiresInSeconds: 60,
    };
  }
  async evidence(a: AuthContext, id: string, token: string) {
    await this.safeguardingAccess(a);
    const schoolId = this.school(a);
    if (!verifyAttachment(token, 'safety-evidence', id, schoolId))
      throw new ForbiddenException('Evidence link is invalid or expired');
    const report = await this.prisma.safetyReport.findFirst({ where: { id, schoolId } });
    if (!report?.evidenceEncrypted) throw new NotFoundException('Evidence not found');
