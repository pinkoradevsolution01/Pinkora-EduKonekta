...
export class SchoolReportsService {
  ...
  private range(field: string, dates: DateRange) {
  ...
  async overview(actor: AuthContext, dates: DateRange = {}) {
    ...
    const created = this.range('createdAt', dates);
    const attendanceDate = this.range('attendanceDate', dates);
    const [
      activeUsers,
      classes,
      enrollments,
      ...
      this.prisma.announcementAcknowledgement.count({
        where: { announcement: { schoolId, ...created } },
      }),
      this.prisma.safetyReport.groupBy({
        by: ['status'],
        where: { schoolId, ...created },
        _count: { _all: true },
      }),
      this.prisma.guidanceCase.count({
        where: { schoolId, status: { in: ['OPEN', 'IN_PROGRESS'] } },
      }),
    ]);
    return {
      activeUsers,
      classes: { total: classes, activeEnrollments: enrollments },
      attendance: attendance.map((item) => ({ state: item.state, total: item._count._all })),
      assignmentCompletion: {
        assignments,
        submitted,
        pending: Math.max(0, assignments - submitted),
      },
      announcementReach: {
        announcements,
        reads: announcementReads,
        parentAcknowledgements: acknowledgements,
      },
    ...
    switch (kind) {
      case 'attendance':
        return this.prisma.attendanceRecord.findMany({
          ...
          orderBy: [{ attendanceDate: 'desc' }, { classId: 'asc' }],
          take,
        });
      case 'assignments':
      ...
      case 'users':
        return this.prisma.schoolMembership.findMany({
          where: { schoolId, ...this.range('joinedAt', dates) },
      ...
      case 'communications':
        ...
        ]).then(([announcements, conversations]) => ({
          announcements,
          conversationMessageTotals: conversations,
        }));
      case 'audit':
  ...
  async export(actor: AuthContext, kind: ReportKind, dates: DateRange = {}) {
    ...
    // Exported reports contain the same scoped, de-identified projections as the viewer.
    // Prefixing spreadsheet formula characters prevents CSV formula injection.
    ...
    const flatten = (item: Record<string, unknown>, prefix = ''): Record<string, string> =>
      Object.entries(item).reduce(
        (result, [key, current]) => {
          ...
          if (
            ...
            Object.assign(result, flatten(current as Record<string, unknown>, name));
          else result[name] = value(current);
          return result;
        },
    ...
    const content = [columns, ...records.map((row) => columns.map((column) => row[column] ?? ''))]
      .map((row) => row.map(quote).join(','))
      .join('\r\n');
    return { fileName: `${kind}-report.csv`, contentType: 'text/csv; charset=utf-8', content };
  }
}
